...
\begin{lemma} \zlabel{thm:Pareto}
    ...
    \begin{enumerate}
        ...
        \item Suppose that~$F_i$ is strictly convex for any~$i = 1, \dots, m$. Then, every Pareto stationary points of~\zcref{eq:MOO} is Pareto optimal for~\zcref{eq:MOO}. \zlabel{thm:Pareto:strict_convex}
    \end{enumerate}
\end{lemma}
\begin{proof}
    ...
    By the definition~\zcref{eq:dir_deriv} of the directional derivative, for a sufficiently small scalar~$\alpha > 0$, we obtain
    \begin{equation}
