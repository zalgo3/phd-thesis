...
\end{proof}
While~$u_\infty$ and~$u_\alpha$ given by~\zcref{eq:gap_MO,eq:reg_gap_MO} are merit functions in the sense of weak Pareto optimality,~$w_\alpha$ defined by~\zcref{eq:reg_lin_gap_MO} is a merit function only in the sense of Pareto stationarity.
...
.\end{equation} 
Therefore, for any~$x \in C$ there exists~$\lambda(x)$ belonging to the unit $m$-simplex~$\simplex^m$ defined by~\zcref{eq:simplex} such that~$\lambda_j(x) = 0$ for all~$j \notin \mathcal{I}_\alpha(x)$ and
...
\begin{proof}
    ...
    Adding the two inequalities gotten by substituting~$(x, z) = (x^1, W_\alpha\left(x^2\right))$ and~$(x, z) = (x^2, W_\alpha\left(x^1\right))$ into~\zcref{eq:reg_lin_gap_optimality}, we obtain
    \begin{align}
             ...
             \le{}& \sum_{i = 1}^{m} \lambda_i\left(x^2\right) \left[ \innerp*{\nabla f_i\left(x^1\right)}{x^1 - W_\alpha\left(x^1\right)} + g_i\left(x^1\right) - g_i\left(W_\alpha\left(x^1\right)\right) \right] \\
              ...
              \begin{align} 
                  ...
                  \le{}& \norm*{x^1 - W_\alpha\left(x^1\right)}_2 \max_{i = 1, \dots, m} \norm*{\nabla f_i\left(x^1\right) - \nabla f_i\left(x^2\right)}_2 \\
                   ...
                   ,\end{align} 
                   where the first inequality comes from~\zcref{eq:min_diff_ineq}, and the third inequality follows from the Cauchy-Schwarz inequality.
                   ...
                   \begin{alignedat}{2}
                       w_\alpha(x) &= \min_{\lambda \in \Delta^m} &&\left\{ \sum_{i = 1}^{m} \lambda_i g_i(x) + \frac{1}{2 \alpha} \norm*{\sum_{i = 1}^{m} \lambda_i \nabla f_i(x)}_2^2 \right. \\
                           ...
                           \begin{theorem} \zlabel{thm:differentiability of the objective function of w_alpha}
                               The objective function of~\zcref{eq:dual_reg_lin_gap} is continuously differentiable at every~$\lambda \in \setR^m$ and
                               \begin{multline}
                           ...
                           \end{proof}
                           Thus, like~\zcref{eq:dual_reg_gap}, \zcref{eq:dual_reg_lin_gap} is solvable with convex optimization techniques such as the interior point method~\cite{Bertsekas1999} when we can quickly evaluate~$\prox_{\alpha \sum_{i = 1}^{m} \lambda_i g_i + \indicator_C}(\cdot)$.
                           When~$n \gg m$, this usually gives a faster way to compute~$w_\alpha$.
                           \begin{example}
                               \begin{enumerate}
                                   \item If~$g_i(x) = 0$ for all~$i = 1, \dots, m$, then~$\prox_{\alpha \sum_{i = 1}^{m} \lambda_i g_i + \indicator_C}$ reduces to the projection onto~$C$ from~\zcref{eq:prox_ind}.
                           ...
                           \begin{theorem} \zlabel{thm:smooth_reg_lin_gap}
                               ...
                               \end{multline}
                               for all~$z \in C$, where~$\prox$ and~$\Lambda$ is given by~\zcref{eq:prox,eq:Pi}, respectively, and~$I_n \in \setR^{n \times n}$ is the~$n$-dimensional identity matrix.
                           ...
                           \begin{corollary} \zlabel{thm:composite_reg_lin_gap}
                               ...
                               If~$\Lambda(x)$ given by~\zcref{eq:Pi} is a singleton, i.e.,~$\Lambda(x) = \set*{\lambda(x)}$, then the function~$w_\alpha - g_1$ is continuously differentiable at~$x$, and we have
                               \begin{multline}
                           ...
                           \end{theorem}
                           \begin{proof}
                               Let~$z \in C$ and $\lambda \in \Lambda(x)$, where~$\Lambda(x)$ is defined by~\zcref{eq:Pi}.
                               ...
                               .\end{multline}
                               Therefore, with similar arguments used in the proof of~\zcref{thm:stationary_reg_gap}, we obtain~$x = \prox_{\alpha \sum_{i = 1}^{m} \lambda_i g_i + \indicator_C}\left( x - \alpha \sum_{i = 1}^{m} \lambda_i \nabla f_i(x) \right)$, and thus~$w_\alpha(x) = 0$.
