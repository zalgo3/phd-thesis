...
This section compares the performance between \zcref{alg:acc_pgm_MO} with various~$a$ and~$b$ and \zcref{alg:acc_pgm_MO} ($a = 0, b = 1 / 4$) through numerical experiments.
...
First, we solve the multi-objective test problems in the form~\zcref{eq:MOO} used in~\cite{Tanabe2022a}, modifications from~\cite{Jin2001,Fliege2009}, whose objective functions are defined by
\begin{align}
    &f_1(x) = \frac{1}{n} \norm*{x}^2,
    f_2(x) = \frac{1}{n} \norm*{x - 2}^2,
...
Furthermore, at each iteration, we transform the subproblem~\zcref{eq:acc prox subprob} into their dual as suggested in~\cite{Tanabe2022a} and solve them with the trust-region interior point method~\cite{Byrd1999} using the scientific library SciPy.
