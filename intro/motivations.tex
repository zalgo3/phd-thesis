...
On the other hand, many previous studies on multi-objective optimization, particularly on the descent methods described in \zcref{sec:intro:multi_objective:descent} and the merit functions described in \zcref{sec:intro:merit:MO}, have dealt with smooth problems, and there is still room for exploration for non-smooth problems.
The projected subgradient method inroduced in \zcref{ex:descent} can handle non-smooth multi-objective optimization, but it may not work well for large-scale problems due to the stepsize decay.
