...
When~$f$ is convex, the accelerated proximal gradient method, also known as Fast Iterative Shrinkage-Thresholding Algorithm (FISTA)~\cite{Beck2009}, can solve~\zcref{eq:composite} with an~$O(1 / k^2)$ rate of convergence, while the proximal gradient method achieves a rate of~$O(1 / k)$.
...
With \zcref{alg:acc_pgm}, $\set{F(x^k) - F(x^\ast)}$ for the global minima~$x^\ast$ converges to zero with a rate of~$O(1 / k^2)$~\cite{Beck2009}, but the convergence of iterates remains unknown.
