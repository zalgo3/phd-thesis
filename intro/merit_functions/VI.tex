...
.\end{equation} 
In particular, if~$T$ is affine, the we call~\zcref{eq:CP} the \emph{linear complementarity problem} (LCP).
    ...
    \begin{description}
        \item[The classical gap function~\cite{Auslender1976,Hearn1982}]
            ...
            \begin{itemize}
                ...
                \item $G_0(x) = 0$ and~$x \in C$ if and only if~$x$ satisfies~\zcref{eq:VI};
                \item If~$C$ is bounded,~$G_0$ is finite everywhere.
            \end{itemize}
            The top two indecate that~$G_0$ is a merit function for the VI~\zcref{eq:VI}.
        \item[The regularized gap function~\cite{Fukushima1992,Auchmuty1989}]
            ...
            Since~\zcref{eq:reg_gap} maximizes a strongly concave function on a nonempty, closed, and convex set, even if~$C$ is unbounded, an unique point attains the maximum, and~$G_\alpha$ is finite everywhere.
